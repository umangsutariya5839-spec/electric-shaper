...
import { createLightDecorationItem, updateLightDecorationItem, deleteLightDecorationItem } from '@/app/actions/cms'
...
export default function LightDecorationClient({ initialItems }: { initialItems: any[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* Add Form */}
      <div className="card">
        <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem' }}>Add New Light Decoration Item</h2>
        <form action={async (formData) => {
          ...
          if (title && imagePath) {
            await createLightDecorationItem({ title, description, category, imagePath, order: Number.isNaN(order) ? 0 : order, isActive, isFeatured })
            ...
            document.getElementById('add-decoration-form')?.closest('form')?.reset()
          }
        }} id="add-decoration-form" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '600px' }}>
          <div>
          ...
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '8px', border: '1px solid var(--color-border)', borderRadius: '4px' }}
              placeholder="/images/light-decoration/1.jpg or https://..."
            />
          </div>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <label htmlFor="decoration-add-active" style={{ fontWeight: '500' }}>Active (visible on website)</label>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <input type="checkbox" id="decoration-add-featured" name="isFeatured" style={{ width: '18px', height: '18px' }} />
            <label htmlFor="decoration-add-featured" style={{ fontWeight: '500' }}>Featured (show in the Home Page light decoration section first)</label>
          </div>
      ...
      {/* Item List */}
      <div className="card">
        <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem' }}>Existing Light Decoration Items</h2>
        {initialItems.length === 0 ? (
          <p style={{ color: 'var(--color-text-muted)' }}>No light decoration items added yet.</p>
        ) : (
              ...
              <div key={item.id} style={{ border: '1px solid var(--color-border)', borderRadius: '4px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                {editingId === item.id ? (
                  ...
                  <form action={async (formData) => {
                    ...
                    const isFeatured = formData.get('isFeatured') === 'on'
                    if (title && imagePath) {
                      await updateLightDecorationItem(item.id, { title, description, category, imagePath, order: Number.isNaN(order) ? 0 : order, isActive, isFeatured })
                      setEditingId(null)
                    }
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <label style={{ fontSize: '0.875rem', fontWeight: '500' }}>Featured (Home Page light decoration)</label>
                    </div>
                    ...
                    <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          ...
                          <form action={async () => { await deleteLightDecorationItem(item.id) }}>
